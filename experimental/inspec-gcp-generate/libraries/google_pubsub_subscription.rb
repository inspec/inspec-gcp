  ...
  def un_parse
    {
      ...
      'topic' => ->(x, path) { x.nil? ? [] : ["its('topic') { should cmp #{x.inspect} }"] },
      'labels' => ->(x, path) { x.nil? ? [] : x.map { |k, v| "its('labels') { should include(#{k.inspect} => #{v.inspect}) }" } },
      'push_config' => ->(x, path) { x.nil? ? [] : GoogleInSpec::Pubsub::Property::SubscriptionPushConfig.un_parse(x, "push_config") },
      'ack_deadline_seconds' => ->(x, path) { x.nil? ? [] : ["its('ack_deadline_seconds') { should cmp #{x.inspect} }"] },
      'message_retention_duration' => ->(x, path) { x.nil? ? [] : ["its('message_retention_duration') { should cmp #{x.inspect} }"] },
      'retain_acked_messages' => ->(x, path) { x.nil? ? [] : ["its('retain_acked_messages') { should cmp #{x.inspect} }"] },
      'expiration_policy' => ->(x, path) { x.nil? ? [] : GoogleInSpec::Pubsub::Property::SubscriptionExpirationPolicy.un_parse(x, "expiration_policy") },
    }
  end
