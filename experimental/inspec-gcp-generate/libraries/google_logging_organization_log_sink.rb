...
class LoggingOrganizationLogSink < GcpResourceBase
  ...
  def un_parse
    {
      'organization' => ->(x, path) { x.nil? ? [] : ["its('organization') { should cmp #{x.inspect} }"] },
      'name' => ->(x, path) { x.nil? ? [] : ["its('name') { should cmp #{x.inspect} }"] },
      'filter' => ->(x, path) { x.nil? ? [] : ["its('filter') { should cmp #{x.inspect} }"] },
      'destination' => ->(x, path) { x.nil? ? [] : ["its('destination') { should cmp #{x.inspect} }"] },
      'writer_identity' => ->(x, path) { x.nil? ? [] : ["its('writer_identity') { should cmp #{x.inspect} }"] },
      'include_children' => ->(x, path) { x.nil? ? [] : ["its('include_children') { should cmp #{x.inspect} }"] },
    }
  end
