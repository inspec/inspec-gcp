  ...
  def un_parse
    {
      ...
      'last_attach_timestamp' => ->(x, path) { x.nil? ? [] : ["its('last_attach_timestamp.to_s') { should cmp '#{x.inspect}' }"] },
      'last_detach_timestamp' => ->(x, path) { x.nil? ? [] : ["its('last_detach_timestamp.to_s') { should cmp '#{x.inspect}' }"] },
      'labels' => ->(x, path) { x.nil? ? [] : x.map { |k, v| "its('labels') { should include(#{k.inspect} => #{v.inspect}) }" } },
      'licenses' => ->(x, path) { x.nil? ? [] : x.map { |single| "its('licenses') { should include #{single.inspect} }" } },
      'name' => ->(x, path) { x.nil? ? [] : ["its('name') { should cmp #{x.inspect} }"] },
      'size_gb' => ->(x, path) { x.nil? ? [] : ["its('size_gb') { should cmp #{x.inspect} }"] },
      'users' => ->(x, path) { x.nil? ? [] : x.map { |single| "its('users') { should include #{single.inspect} }" } },
      'physical_block_size_bytes' => ->(x, path) { x.nil? ? [] : ["its('physical_block_size_bytes') { should cmp #{x.inspect} }"] },
      'type' => ->(x, path) { x.nil? ? [] : ["its('type') { should cmp #{x.inspect} }"] },
      'source_image' => ->(x, path) { x.nil? ? [] : ["its('source_image') { should cmp #{x.inspect} }"] },
      'zone' => ->(x, path) { x.nil? ? [] : ["its('zone') { should cmp #{x.inspect} }"] },
      'source_image_encryption_key' => ->(x, path) { x.nil? ? [] : GoogleInSpec::Compute::Property::DiskSourceImageEncryptionKey.un_parse(x, "source_image_encryption_key") },
      'source_image_id' => ->(x, path) { x.nil? ? [] : ["its('source_image_id') { should cmp #{x.inspect} }"] },
      'disk_encryption_key' => ->(x, path) { x.nil? ? [] : GoogleInSpec::Compute::Property::DiskDiskEncryptionKey.un_parse(x, "disk_encryption_key") },
      'source_snapshot' => ->(x, path) { x.nil? ? [] : ["its('source_snapshot') { should cmp #{x.inspect} }"] },
      'source_snapshot_encryption_key' => ->(x, path) { x.nil? ? [] : GoogleInSpec::Compute::Property::DiskSourceSnapshotEncryptionKey.un_parse(x, "source_snapshot_encryption_key") },
      'source_snapshot_id' => ->(x, path) { x.nil? ? [] : ["its('source_snapshot_id') { should cmp #{x.inspect} }"] },
    }
  end
