...
class ComputeBackendBucket < GcpResourceBase
  ...
  def un_parse
    {
      'bucket_name' => ->(x, path) { x.nil? ? [] : ["its('bucket_name') { should cmp #{x.inspect} }"] },
      'cdn_policy' => ->(x, path) { x.nil? ? [] : GoogleInSpec::Compute::Property::BackendBucketCdnPolicy.un_parse(x, "cdn_policy") },
