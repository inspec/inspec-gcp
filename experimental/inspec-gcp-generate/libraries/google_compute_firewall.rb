...
class ComputeFirewall < GcpResourceBase
  ...
  def un_parse
    {
      'allowed' => ->(x, path) { x.nil? ? [] : x.map { |single| "its('allowed') { should include '#{single.to_json}' }" } },
      'creation_timestamp' => ->(x, path) { x.nil? ? [] : ["its('creation_timestamp.to_s') { should cmp '#{x.inspect}' }"] },
      'denied' => ->(x, path) { x.nil? ? [] : x.map { |single| "its('denied') { should include '#{single.to_json}' }" } },
      'description' => ->(x, path) { x.nil? ? [] : ["its('description') { should cmp #{x.inspect} }"] },
      'destination_ranges' => ->(x, path) { x.nil? ? [] : x.map { |single| "its('destination_ranges') { should include #{single.inspect} }" } },
      'direction' => ->(x, path) { x.nil? ? [] : ["its('direction') { should cmp #{x.inspect} }"] },
      'disabled' => ->(x, path) { x.nil? ? [] : ["its('disabled') { should cmp #{x.inspect} }"] },
      ...
      'source_ranges' => ->(x, path) { x.nil? ? [] : x.map { |single| "its('source_ranges') { should include #{single.inspect} }" } },
      'source_service_accounts' => ->(x, path) { x.nil? ? [] : x.map { |single| "its('source_service_accounts') { should include #{single.inspect} }" } },
      'source_tags' => ->(x, path) { x.nil? ? [] : x.map { |single| "its('source_tags') { should include #{single.inspect} }" } },
      'target_service_accounts' => ->(x, path) { x.nil? ? [] : x.map { |single| "its('target_service_accounts') { should include #{single.inspect} }" } },
      'target_tags' => ->(x, path) { x.nil? ? [] : x.map { |single| "its('target_tags') { should include #{single.inspect} }" } },
    }
  end
