  ...
  def un_parse
    {
      ...
      'quic_override' => ->(x, path) { x.nil? ? [] : ["its('quic_override') { should cmp #{x.inspect} }"] },
      'ssl_certificates' => ->(x, path) { x.nil? ? [] : x.map { |single| "its('ssl_certificates') { should include #{single.inspect} }" } },
      'ssl_policy' => ->(x, path) { x.nil? ? [] : ["its('ssl_policy') { should cmp #{x.inspect} }"] },
      'url_map' => ->(x, path) { x.nil? ? [] : ["its('url_map') { should cmp #{x.inspect} }"] },
    }
  end
