  ...
  def un_parse
    {
      ...
      'substitutions' => ->(x, path) { x.nil? ? [] : x.map { |k, v| "its('substitutions') { should include(#{k.inspect} => #{v.inspect}) }" } },
      'filename' => ->(x, path) { x.nil? ? [] : ["its('filename') { should cmp #{x.inspect} }"] },
      'ignored_files' => ->(x, path) { x.nil? ? [] : x.map { |single| "its('ignored_files') { should include #{single.inspect} }" } },
      'included_files' => ->(x, path) { x.nil? ? [] : x.map { |single| "its('included_files') { should include #{single.inspect} }" } },
      'trigger_template' => ->(x, path) { x.nil? ? [] : GoogleInSpec::CloudBuild::Property::TriggerTriggerTemplate.un_parse(x, "trigger_template") },
      'build' => ->(x, path) { x.nil? ? [] : GoogleInSpec::CloudBuild::Property::TriggerBuild.un_parse(x, "build") },
    }
  end
