  ...
  def un_parse
    {
      ...
      'profile' => ->(x, path) { x.nil? ? [] : ["its('profile') { should cmp #{x.inspect} }"] },
      'min_tls_version' => ->(x, path) { x.nil? ? [] : ["its('min_tls_version') { should cmp #{x.inspect} }"] },
      'enabled_features' => ->(x, path) { x.nil? ? [] : x.map { |single| "its('enabled_features') { should include #{single.inspect} }" } },
      'custom_features' => ->(x, path) { x.nil? ? [] : x.map { |single| "its('custom_features') { should include #{single.inspect} }" } },
      'fingerprint' => ->(x, path) { x.nil? ? [] : ["its('fingerprint') { should cmp #{x.inspect} }"] },
      'warnings' => ->(x, path) { x.nil? ? [] : x.map { |single| "its('warnings') { should include '#{single.to_json}' }" } },
    }
  end
