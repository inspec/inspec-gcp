  ...
  def un_parse
    {
      ...
      'config' => ->(x, path) { x.nil? ? [] : GoogleInSpec::Container::Property::RegionalNodePoolConfig.un_parse(x, "config") },
      ...
      'version' => ->(x, path) { x.nil? ? [] : ["its('version') { should cmp #{x.inspect} }"] },
      'autoscaling' => ->(x, path) { x.nil? ? [] : GoogleInSpec::Container::Property::RegionalNodePoolAutoscaling.un_parse(x, "autoscaling") },
      'management' => ->(x, path) { x.nil? ? [] : GoogleInSpec::Container::Property::RegionalNodePoolManagement.un_parse(x, "management") },
      'max_pods_constraint' => ->(x, path) { x.nil? ? [] : GoogleInSpec::Container::Property::RegionalNodePoolMaxPodsConstraint.un_parse(x, "max_pods_constraint") },
      'conditions' => ->(x, path) { x.nil? ? [] : x.map { |single| "its('conditions') { should include '#{single.to_json}' }" } },
      'pod_ipv4_cidr_size' => ->(x, path) { x.nil? ? [] : ["its('pod_ipv4_cidr_size') { should cmp #{x.inspect} }"] },
      'cluster' => ->(x, path) { x.nil? ? [] : ["its('cluster') { should cmp #{x.inspect} }"] },
