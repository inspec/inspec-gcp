  ...
  def un_parse
    {
      ...
      'deprecated' => ->(x, path) { x.nil? ? [] : GoogleInSpec::Compute::Property::RegionDeprecated.un_parse(x, "deprecated") },
      ...
      'quotas' => ->(x, path) { x.nil? ? [] : x.map { |single| "its('quotas') { should include '#{single.to_json}' }" } },
      'status' => ->(x, path) { x.nil? ? [] : ["its('status') { should cmp #{x.inspect} }"] },
      'zones' => ->(x, path) { x.nil? ? [] : x.map { |single| "its('zones') { should include #{single.inspect} }" } },
    }
  end
