...
class BigQueryTable < GcpResourceBase
  ...
  def un_parse
    {
      'table_reference' => ->(x, path) { x.nil? ? [] : GoogleInSpec::BigQuery::Property::TableTableReference.un_parse(x, "table_reference") },
      'clustering' => ->(x, path) { x.nil? ? [] : x.map { |single| "its('clustering') { should include #{single.inspect} }" } },
      ...
      'num_bytes' => ->(x, path) { x.nil? ? [] : ["its('num_bytes') { should cmp #{x.inspect} }"] },
      'num_long_term_bytes' => ->(x, path) { x.nil? ? [] : ["its('num_long_term_bytes') { should cmp #{x.inspect} }"] },
      'num_rows' => ->(x, path) { x.nil? ? [] : ["its('num_rows') { should cmp #{x.inspect} }"] },
      'require_partition_filter' => ->(x, path) { x.nil? ? [] : ["its('require_partition_filter') { should cmp #{x.inspect} }"] },
      'type' => ->(x, path) { x.nil? ? [] : ["its('type') { should cmp #{x.inspect} }"] },
      'view' => ->(x, path) { x.nil? ? [] : GoogleInSpec::BigQuery::Property::TableView.un_parse(x, "view") },
      'time_partitioning' => ->(x, path) { x.nil? ? [] : GoogleInSpec::BigQuery::Property::TableTimePartitioning.un_parse(x, "time_partitioning") },
      'streaming_buffer' => ->(x, path) { x.nil? ? [] : GoogleInSpec::BigQuery::Property::TableStreamingBuffer.un_parse(x, "streaming_buffer") },
      'schema' => ->(x, path) { x.nil? ? [] : GoogleInSpec::BigQuery::Property::TableSchema.un_parse(x, "schema") },
      'encryption_configuration' => ->(x, path) { x.nil? ? [] : GoogleInSpec::BigQuery::Property::TableEncryptionConfiguration.un_parse(x, "encryption_configuration") },
      'expiration_time' => ->(x, path) { x.nil? ? [] : ["its('expiration_time') { should cmp #{x.inspect} }"] },
      'external_data_configuration' => ->(x, path) { x.nil? ? [] : GoogleInSpec::BigQuery::Property::TableExternalDataConfiguration.un_parse(x, "external_data_configuration") },
      'dataset' => ->(x, path) { x.nil? ? [] : ["its('dataset') { should cmp #{x.inspect} }"] },
    }
  end
