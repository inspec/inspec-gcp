  ...
  def un_parse
    {
      ...
      'disk_size_gb' => ->(x, path) { x.nil? ? [] : ["its('disk_size_gb') { should cmp #{x.inspect} }"] },
      'name' => ->(x, path) { x.nil? ? [] : ["its('name') { should cmp #{x.inspect} }"] },
      'description' => ->(x, path) { x.nil? ? [] : ["its('description') { should cmp #{x.inspect} }"] },
      'storage_bytes' => ->(x, path) { x.nil? ? [] : ["its('storage_bytes') { should cmp #{x.inspect} }"] },
      ...
      'source_disk' => ->(x, path) { x.nil? ? [] : ["its('source_disk') { should cmp #{x.inspect} }"] },
      'zone' => ->(x, path) { x.nil? ? [] : ["its('zone') { should cmp #{x.inspect} }"] },
      'snapshot_encryption_key' => ->(x, path) { x.nil? ? [] : GoogleInSpec::Compute::Property::SnapshotSnapshotEncryptionKey.un_parse(x, "snapshot_encryption_key") },
      'source_disk_encryption_key' => ->(x, path) { x.nil? ? [] : GoogleInSpec::Compute::Property::SnapshotSourceDiskEncryptionKey.un_parse(x, "source_disk_encryption_key") },
    }
  end
