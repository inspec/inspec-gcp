  ...
  def un_parse
    {
      ...
      'ttl' => ->(x, path) { x.nil? ? [] : ["its('ttl') { should cmp #{x.inspect} }"] },
      'target' => ->(x, path) { x.nil? ? [] : x.map { |single| "its('target') { should include #{single.inspect} }" } },
      'managed_zone' => ->(x, path) { x.nil? ? [] : ["its('managed_zone') { should cmp #{x.inspect} }"] },
    }
  end
