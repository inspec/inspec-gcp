  ...
  def un_parse
    {
      ...
      'default_service' => ->(x, path) { x.nil? ? [] : ["its('default_service') { should cmp #{x.inspect} }"] },
      'description' => ->(x, path) { x.nil? ? [] : ["its('description') { should cmp #{x.inspect} }"] },
      'host_rules' => ->(x, path) { x.nil? ? [] : x.map { |single| "its('host_rules') { should include '#{single.to_json}' }" } },
      ...
      'path_matchers' => ->(x, path) { x.nil? ? [] : x.map { |single| "its('path_matchers') { should include '#{single.to_json}' }" } },
      'tests' => ->(x, path) { x.nil? ? [] : x.map { |single| "its('tests') { should include '#{single.to_json}' }" } },
    }
  end
