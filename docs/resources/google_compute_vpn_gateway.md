...
Use the `google_compute_vpn_gateway` InSpec audit resource to test the properties of a Google Cloud VpnGateway resource.
...
describe google_compute_vpn_gateway(project: 'chef-gcp-inspec', region: ' value_region', vpnGateway: ' ') do
	...
	its('gateway_ip_version') { should cmp '' }
...
Properties that can be accessed from the `google_compute_vpn_gateway` resource:
  ...
  * `vpn_interfaces`: The list of VPN interfaces associated with this VPN gateway.
    ...
    * `ipv6_address`: [Output Only] IPv6 address for this VPN interface associated with the VPN gateway. The IPv6 address must be a regional external IPv6 address. The format is RFC 5952 format (e.g. 2001:db8::2d9:51:0:0).
  ...
  * `gateway_ip_version`: The IP family of the gateway IPs for the HA-VPN gateway interfaces. If not specified, IPV4 will be used.
  Possible values:
    * IPV4
    * IPV6
