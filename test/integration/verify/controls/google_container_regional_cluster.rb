...
control 'google_container_regional_cluster-1.0' do
  ...
  describe google_container_regional_cluster(project: gcp_project_id, location: gcp_region, name: regional_cluster['name']) do
    it { should exist }
    its('initial_node_count') { should eq regional_cluster['initial_node_count']}
    its('location') { should eq gcp_region }
  end

  describe google_container_regional_cluster(project: gcp_project_id, location: gcp_region, name: 'nonexistent') do
    it { should_not exist }
  end
end
